...
App.initData = function() {
	...
	this.days = data.days;
	this.initCalendar();
	this.activeWorkouts = _.chain(_.flatten(this.days)).where({weeknum:this.weeknum, type:'workout'}).pluck('title').value();
	for( var i = 0; i < this.days.length; i++ ) {
		var template = Handlebars.compile(this.templates.dayTemplate);
		var workouts = _.chain(this.days[i]).where({weeknum:this.weeknum}).value();
...
App.initListeners = function() {
	...
	$(document).on('click', '.dayContent', function(e) { if(e.target != $('.dayContent')[0]) return; that.showTitles(); that.activeDay = parseInt($(this).parent().attr('data-id')); });
	$(document).on('click', '#workout-titles .workout-title', function() { that.addWorkout($(this)) });
	$(document).on('click', '#clear', function() { delete localStorage.woplans2; location.reload(); });
...
App.showWorkout = function( $el ) {
	...
	var workout = _.chain(this.days[this.activeDay]).findWhere({weeknum: this.weeknum, title: $el.attr('data-title')}).value();	
...
App.addWorkout = function( $workout ) {
	...
	this.days[this.activeDay].push( this._data($workout) );
	ds._save(this.days);
};
...
App.removeWorkout = function( $workout ) {
	...
	var d = this.days[data.daynum],
		i = d.indexOf(_.chain(d).findWhere(data).value());
	this.days[data.daynum].splice(i, 1);
...
App.enterCardio = function( $el ) {
	...
	var day = this.days[this.activeDay],
		c = _.chain(day).findWhere({title:title, weeknum:this.weeknum}).value(),
		i = day.indexOf(c);
	this.days[this.activeDay][i] = data;
...
App.enterWeight = function( $weight ) {
	...
	if( lbs != '' && !isNaN(lbs) ) {
		...
		var wo = _.chain(this.days[this.activeDay]).findWhere({title:data.wotitle}).value(),
			i = this.days[this.activeDay].indexOf( wo );
		_.extend(wo.exercises[data.exnum], data);
		this.days[this.activeDay][i] = wo;
